...
class ChangeMasterPasswordDialog(QDialog):
    """Dialog for changing the master password with 2FA verification."""
    ...
    def __init__(self, main_window):
        ...
        buttons = [
            ("Change Password", 140, self.initiate_change_password),
    ...
    def initiate_change_password(self):
        ...
        if not self.main_window.verify_master_password(current_pw):
            self.main_window.show_warning("Current master password is incorrect.")
            logger.warning(
                f"Incorrect master password attempt by user: {self.main_window.user_identifier}"
            )
            return
        ...
        if not two_fa.get_secret():
            self.main_window.show_warning(
                "Two-Factor Authentication is not enabled. Please enable 2FA before changing the master password."
            )
            logger.warning(
                f"User {self.main_window.user_identifier} attempted to change password without 2FA."
            )
            return

        # Prompt for 2FA Token using centralized dialog
        token_dialog = TwoFactorVerificationDialog(self.main_window)
        if token_dialog.exec() == QDialog.Accepted and token_dialog.verified:
            try:
                ...
                if success:
                    ...
                    logger.info(
                        f"Master password changed successfully for user: {self.main_window.user_identifier}"
                    )
                    self.accept()
                else:
                    self.main_window.show_warning(message)
                    logger.warning(
                        f"Failed to change master password for user: {self.main_window.user_identifier} - {message}"
                    )
            except Exception as e:
                self.main_window.show_warning(
                    f"Failed to change master password: {str(e)}"
                )
                logger.error(
                    f"Failed to change master password for user: {self.main_window.user_identifier} - {str(e)}"
                )
        else:
            self.main_window.show_warning(
                "2FA verification failed. Password not changed."
            )
            logger.warning(
                f"2FA verification failed during password change for user: {self.main_window.user_identifier}"
            )


class ChangeEmailDialog(QDialog):
    """Dialog for changing the user's email address with 2FA verification."""
    ...
    def __init__(self, main_window):
        ...
        self.new_email_input.setPlaceholderText("example@domain.com")
        ...
        button_factory = ButtonFactory(self)
        buttons = [
            ("Change Email", 120, self.initiate_change_email),
            ("Cancel", 100, self.reject),
        ]
        button_layout = button_factory.create_buttons_with_spacing(buttons)
        layout.addLayout(button_layout)

    def initiate_change_email(self):
        """Handle the email change process with proper verification."""
        current_pw, ok = QInputDialog.getText(
            self,
            "Change Email",
            "Enter your current master password:",
            QLineEdit.Password,
        )
        if ok and current_pw:
            if not self.main_window.verify_master_password(current_pw):
                self.main_window.show_warning("Incorrect master password.")
                logger.warning("Incorrect master password attempt during email change.")
                return
            ...
            if ok and new_email:
                ...
                try:
                    ...
                    logger.info(f"Email address updated to: {new_email}")
                except Exception as e:
...
class SettingsTab(QWidget):
    ...
    def load_trusted_devices(self):
        ...
        for device in trusted_devices:
            ...
            # Display 'Alias' and 'Added On'
            alias_item = QTableWidgetItem(device.get("alias", "Unnamed Device"))
            alias_item.setData(
                Qt.UserRole, device.get("device_id")
            )  # Store device_id as hidden data
    ...
    def revoke_selected_devices(self):
        ...
        for selected_row in selected_rows:
            alias_item = self.trusted_devices_table.item(selected_row.row(), 0)
            device_id = (
                alias_item.data(Qt.UserRole) if alias_item else None
            )  # Retrieve device_id from hidden data
            ...
            if device_id:
                ...
                logger.warning(
                    "Attempted to revoke a device without a valid device_id."
                )
                self.main_window.show_warning(
                    "Failed to revoke selected device due to invalid data."
                )
    ...
    def show_qr_code(self, qr_code_image_data: bytes):
        ...
        layout = QVBoxLayout()
        instructions = QLabel(
            "Scan this QR code with your authenticator app then close this window."
        )
...
class TwoFactorVerificationDialog(QDialog):
    """Dialog for verifying the 2FA token."""

    def __init__(self, main_window):
        super().__init__(main_window)
        self.main_window = main_window
        self.setWindowTitle("Two-Factor Authentication")
        self.setWindowIcon(QIcon(r"frontend/icons/2fa_icon.png"))
        self.setMinimumSize(300, 150)

        layout = QVBoxLayout(self)

        self.token_label = QLabel("Enter your 2FA Token:")
        self.token_input = QLineEdit()
        self.token_input.setPlaceholderText("123456")
        self.token_input.setMaxLength(6)
        self.token_input.setValidator(QIntValidator())
    ...
    def verify_token(self):
        ...
        if not token:
            self.main_window.show_warning("2FA token is required.")
            return
        ...
        try:
            if two_fa.verify_token(token):
                self.verified = True
                logger.info(
                    f"2FA token verified successfully for user: {self.main_window.user_identifier}"
                )
                self.accept()
            else:
                self.main_window.show_warning("Invalid 2FA token.")
                logger.warning(
                    f"Invalid 2FA token attempt for user: {self.main_window.user_identifier}"
                )
        except Exception as e:
            self.main_window.show_warning(f"Failed to verify 2FA token: {str(e)}")
            logger.error(
                f"Failed to verify 2FA token for user: {self.main_window.user_identifier} - {str(e)}"
            )
